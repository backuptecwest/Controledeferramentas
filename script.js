...
// --- LÓGICA DE INICIALIZAÇÃO ROBUSTA (v4.9) ---
...
function initializeGapiClient() {
    ...
    // Tenta inicializar a app caso o GIS já tenha carregado
    maybeInit();
}
...
function initializeGisClient() {
    ...
    // Tenta inicializar a app caso o GAPI já tenha carregado
    maybeInit();
}

let initCalled = false; // Flag para garantir que maybeInit só roda uma vez
async function maybeInit() {
    if (gapiClientPromise && gisClient && !initCalled) {
        initCalled = true; // Previne execuções múltiplas
        try {
            await gapiClientPromise;
            gisClient.requestAccessToken({ prompt: 'none' }); // Tenta o login silencioso
        } catch (error) {
...
async function handleAuthResponse(tokenResponse) {
    if (tokenResponse.error) {
        console.log("Login silencioso falhou, mostrando botão de login.");
...
function showLoadingScreen() { document.getElementById('main-app-content').style.display = 'none'; document.getElementById('login-container').style.display = 'block'; document.getElementById('authorize_button').style.display = 'none'; document.getElementById('login-message').innerText = "A carregar..."; }
...
// A função window.onload agora é substituída pela lógica de inicialização no topo.
// Os event listeners dos botões são ativados na nova função activateAppButtons().