...
function gisLoaded() {
    ...
    gisInited = true;
    checkTokenAndLogin(); // NOVO: Verifica o token ao carregar
}
...
// NOVO: Função central que decide se faz login automático ou mostra o botão
function checkTokenAndLogin() {
    if (gapiInited && gisInited) {
        // Tenta fazer o login silenciosamente
        tokenClient.requestAccessToken({ prompt: 'none' });
    }
}

function handleAuthClick() {
    // Se o login silencioso falhar, o utilizador clica aqui
    tokenClient.requestAccessToken({ prompt: 'consent' });
}

async function handleAuthResponse(resp) {
    if (resp.error) {
        // O login silencioso falhou, mostra o botão de login manual
        document.getElementById('login-message').style.display = 'none';
        document.getElementById('authorize_button').style.display = 'block';
        return;
    }
    // Esconde o ecrã de login e mostra a app
    document.getElementById('login-container').style.display = 'none';
    document.getElementById('main-app-content').style.display = 'block';
    await onLogin();
}

function handleSignoutClick() { const token = gapi.client.getToken(); if (token !== null) { google.accounts.oauth2.revoke(token.access_token); gapi.client.setToken(''); document.getElementById('main-app-content').style.display = 'none'; document.getElementById('login-container').style.display = 'block'; document.getElementById('login-message').style.display = 'none'; document.getElementById('authorize_button').style.display = 'block'; driveFileId = null; } }

async function onLogin() { /* ... código sem alterações ... */ }
async function findOrCreateDataFile() { /* ... código sem alterações ... */ }
async function saveDataToDrive() { /* ... código sem alterações ... */ }

// NOVO: Função para o botão "Atualizar Dados"
async function refreshData() {
    console.log("A atualizar dados do Google Drive...");
    await loadDataFromDrive();
    updateOperationSelects();
    // Se alguma modal estiver aberta, atualiza o seu conteúdo também
    const openModal = document.querySelector('.modal[style*="display: block"]');
    if (openModal) {
        openModal.style.display = 'none'; // Fecha para forçar a reabertura com dados novos
        openModal.style.display = 'block';
        // Re-chama a função de atualização da modal específica
        switch(openModal.id) {
            case 'tools-modal': updateManagementLists(); break;
            case 'techs-modal': updateManagementLists(); break;
            case 'history-modal': updateHistoryLog(); break;
            case 'status-modal': updateFullStatusList(); break;
        }
    }
    console.log("Dados atualizados.");
}

async function loadDataFromDrive() { /* ... código sem alterações ... */ }

// #############################################################
// #####   COLE AQUI O RESTANTE DAS FUNÇÕES DO SCRIPT.JS   #####
// #####   (A partir de `async function onLogin()` até ao final)   #####
// #####   A única função que muda é a `updateFullStatusList`   #####
// #############################################################

// --- Lógica Principal ---
async function onLogin() { await findOrCreateDataFile(); await loadDataFromDrive(); updateOperationSelects(); try { const profile = await gapi.client.oauth2.userinfo.get(); document.getElementById('user-profile').innerText = `Logado como: ${profile.result.name}`; } catch(e) { console.error(e); } }
...
// NOVO: Status Geral agora filtra ferramentas inativas
function updateFullStatusList() {
    const fullStatusList = document.getElementById('full-status-list');
    fullStatusList.innerHTML = '';
    const activeTools = tools.filter(tool => tool.status === 'ativo'); // Filtra aqui!
    if (activeTools.length === 0) {
        fullStatusList.innerHTML = '<li>Nenhuma ferramenta ativa cadastrada.</li>';
        return;
    }
    activeTools.forEach(tool => {
        ...
        if (assignment) {
            ...
            const contextText = assignment.context ? `(Cliente/OS: ${assignment.context})` : '';
            li.innerHTML = `<span>${tool.name}</span> <span class="status status-in-use">EM USO</span> com <strong>${techName}</strong> ${contextText}`;
        } else {
            li.innerHTML = `<span>${tool.name}</span> <span class="status status-available">DISPONÍVEL</span>`;
        }
        fullStatusList.appendChild(li);
    });
}
// --- INICIALIZAÇÃO FINAL ---
window.onload = function() {
    ...
    document.getElementById('refresh-data-btn').onclick = refreshData; // Conecta o novo botão
