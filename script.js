...
// (Copie o objeto inteiro que o Firebase lhe fornece, com todos os campos)
// ##################################################################
...
try {
    ...
    async function handleRegister() {
        ...
        try {
            ...
            if (querySnapshot.empty && email !== auth.currentUser?.email) {
    ...
    // ... (O resto do código é o mesmo da v10.1, cole-o aqui)
    // Para garantir, o bloco completo de funções que faltam está abaixo.
    function listenToDataChanges() { if (!currentUser || !companyId) return; const collectionsToListen = { tools: query(collection(db, "companies", companyId, "tools"), orderBy("name")), techs: query(collection(db, "companies", companyId, "techs"), orderBy("name")), assignments: collection(db, "companies", companyId, "assignments"), history: query(collection(db, "companies", companyId, "history"), orderBy("returnDate", "desc")) }; for (const [col, ref] of Object.entries(collectionsToListen)) { const unsub = onSnapshot(ref, snapshot => { window[col] = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); updateUI(); }); unsubscribes.push(unsub); } if(isAdmin) { const unsub = onSnapshot(query(collection(db, "companies", companyId, "authorized_users")), snapshot => { authorizedUsers = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); if (document.querySelector('#users-modal[style*="display: block"]')) updateUserManagementList(); }); unsubscribes.push(unsub); } }
    function updateUI() { updateOperationSelects(); const openModal = document.querySelector('.modal[style*="display: block"]'); if(openModal) { switch(openModal.id) { case 'tools-modal': case 'techs-modal': updateManagementLists(); break; case 'history-modal': updateHistoryLog(); break; case 'status-report-modal': const title = document.getElementById('status-report-title').textContent; if (title.includes("Disponíveis")) showAvailableToolsList(false); else if (title.includes("em Uso")) showInUseToolsList(false); break; } } }
    ...
    document.body.innerHTML = `<h1>Erro Crítico na Aplicação</h1><p>Ocorreu um erro inesperado. Verifique a consola (F12) para detalhes técnicos. Pode ser necessário verificar o objeto 'firebaseConfig'.</p>`;
}