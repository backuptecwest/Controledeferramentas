...
// --- Funções de Inicialização e Autenticação (Melhoradas) ---
...
async function initializeGapiClient() {
    try {
        ...
        document.getElementById('login-message').innerText = "Erro ao carregar. Tente recarregar a página.";
    }
}
...
async function onLogin() {
    ...
    await loadDataFromDrive(true); // true para forçar atualização da UI
    try {
...
// NOVO: Sincronização Não-Destrutiva
async function refreshData() {
    console.log("Sincronizando dados...");
    await loadDataFromDrive(false); // false para não atualizar a UI principal
    
    // Atualiza apenas as modais, se estiverem abertas
...
async function loadDataFromDrive(updateUI) {
    ...
    try {
        ...
        if (response.body && response.body.length > 0) {
            ...
            tools = []; techs = []; assignments = []; history = [];
        }
        if (updateUI) {
        ...
        if(e.result && e.result.error && e.result.error.message.includes("File not found")){ driveFileId = null; await findOrCreateDataFile(); }
    }
}
...
// --- Funções da Aplicação (com ordenação) ---
function updateManagementLists() {
    const toolList = document.getElementById('tool-management-list');
    toolList.innerHTML = '';
    [...tools].sort(sortByStatusAndName).forEach(tool => {
        const li = document.createElement('li');
        li.className = `item-${tool.status}`;
        li.innerHTML = `<span>${tool.name}</span> <div class="button-group"><button class="small-button" onclick="editTool(${tool.id})">Editar</button><button class="small-button" onclick="toggleToolStatus(${tool.id})">${tool.status === 'ativo' ? 'Inativar' : 'Reativar'}</button></div>`;
        toolList.appendChild(li);
    });
    const techList = document.getElementById('tech-management-list');
    techList.innerHTML = '';
    [...techs].sort(sortByStatusAndName).forEach(tech => {
        const li = document.createElement('li');
        li.className = `item-${tech.status}`;
        li.innerHTML = `<span>${tech.name}</span> <div class="button-group"><button class="small-button" onclick="editTech(${tech.id})">Editar</button><button class="small-button" onclick="toggleTechStatus(${tech.id})">${tech.status === 'ativo' ? 'Inativar' : 'Reativar'}</button></div>`;
...
function updateOperationSelects() {
    ...
    // Guarda seleções atuais para não interromper o utilizador
    const selectedTool = toolSelect.value;
    const selectedReturn = returnToolSelect.value;
    ...
    returnToolSelect.value = selectedReturn;
...
// Cole aqui o resto de TODAS as funções para garantir
