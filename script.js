...
function listenToDataChanges() {
    ...
    const collections = ['tools', 'techs', 'assignments', 'history'];
    collections.forEach(colName => {
        const ref = collection(db, "companies", companyId, colName);
        const unsub = onSnapshot(ref, snapshot => {
            window[colName] = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
function updateOperationSelects() {
    ...
    // Guarda as seleções atuais para não as apagar durante a sincronização
    ...
    [...availableTools].sort(sortByName).forEach(tool => {
        toolSelect.innerHTML += `<option value="${tool.id}">${tool.name}</option>`;
    });
    
    const assignedTools = tools.filter(t => toolsInUseIds.includes(t.id));
    returnToolSelect.innerHTML = '<option value="">Selecione...</option>';
    [...assignedTools].sort(sortByName).forEach(tool => {
        returnToolSelect.innerHTML += `<option value="${tool.id}">${tool.name}</option>`;
    });
    
    const activeTechs = techs.filter(t => t.status === 'ativo');
    techSelect.innerHTML = '<option value="">Selecione...</option>';
    [...activeTechs].sort(sortByName).forEach(tech => {
        techSelect.innerHTML += `<option value="${tech.id}">${tech.name}</option>`;
    });
    
    // Restaura as seleções se ainda forem válidas
...
// O restante do código... (CRUD, UI, etc.)
